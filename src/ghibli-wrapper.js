 ...
 */
export const filterDirector = async (director) => {
 ...
 */
export const filterProducer = async (producer) => {
 ...
 * 
 * @param {number} releaseDate The year the film was released
 * @returns {object} The film(s) released in the given year
 */
export const filterRelease = async (releaseDate) => {
    const filmJSON = await getFilms()
    console.log(filmJSON.filter((film) => {
 ...
 * 
 * @param {number} rt_score The minimum score of a film to return
 * @returns {object} The film(s) that have a score of at least rt_score
 */
export const filterRanking = async (rt_score) => {
    const filmJSON = await getFilms()
